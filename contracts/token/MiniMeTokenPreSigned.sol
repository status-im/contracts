...
contract MiniMeTokenPreSigned is MiniMeToken {
        ...
        public 
    {
        ...
        if (_gasPrice > 0) {
            if (!doTransfer(recovered, msg.sender, _gasPrice*_gas)) {
                revert();
            }    
        }
    }
        ...
        public
    {
        ...
        if (_gasPrice > 0) {
            ...
            if (!doTransfer(recovered, msg.sender, _gasPrice*_gas)) {
                revert(); //reverts the full stack, not just this local execution call.
            }
        }
        ...
        public
    {
        uint256 _gas = msg.gas;
        require(transfersEnabled);
        address recovered = recoverApprovePreSigned(_sigV, _sigR, _sigS, _to, _value, _gasPrice, _nonce);
        require(recovered > 0x0);
    ...
    /**
     * @notice Gets txHash `transfer(address,uint256)`
     * @param _to The address of the recipient
     * @param _value The value of tokens to be transferred
     * @param _gasPrice How much tokens willing to pay per gas
     * @param _nonce Presigned transaction number.
     * @return txHash Presigned `transfer(address,uint256)` hash
     */    
    ...
    /**
     * @notice get txHash of presigned `approveAndCall(address,uint256,bytes)`
     * @param _to The address of the recipient
     * @param _value The value of tokens to be transferred
     * @param _extraData option data to send to contract
     * @param _gasPrice How much tokens willing to pay per gas
     * @param _nonce Presigned transaction number.
     * @return txHash Presigned `approveAndCall(address,uint256,bytes)` Hash
     */
        ...
        returns(bytes32 txHash)
    {
        ...
        txHash = keccak256(address(this), bytes4(0xcae9ca51), _to, _value, _extraData, _gasPrice, _nonce);
    }
        ...
        returns(bytes32 txHash)
    {
        ...
        txHash = keccak256(address(this), bytes4(0x095ea7b3), _to, _value, _gasPrice, _nonce);
    }
    ...
    {
        var (sigV, sigR, sigS) = signatureSplit(_signature);
        approvePreSigned(sigV, sigR, sigS, _to, _value, _gasPrice, _nonce);
    }    
        ...
        returns(address recovered)
    {
     ...
     /**
     * @notice recover the address which signed an approveAndCall
     * @param _sigV Signature V 
     * @param _sigR Signature R
     * @param _sigS Signature S
     * @param _to The address of the recipient
    ...
    function recoverApprovePreSigned(
        ...
        returns (address recovered)
    {
        recovered = ecrecover(getSignHash(getApproveHash(_to, _value, _gasPrice, _nonce)), _sigV, _sigR, _sigS);
    }
